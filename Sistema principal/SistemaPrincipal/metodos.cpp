...
void in_a_state_subsis_menus(){
  ...
  if(millis() - tiempoLecturaBME > 250){
    ...
    hBME = bme.readHumidity();    //Leemos la Humedad
    tBME = bme.readTemperature();  //Leemos la temperatura en grados Celsius
    lcd.setCursor(0, 0 );
    // lcd.print("Hume: ");
    // Serial.println(tBME);
    // // Serial.println(tBME);
...
void in_b_state_subsis_temp(){
  ...
  // tBME = bme.readTemperature(); Esto estaba muy miy mal en el programa, se mamaba la comunicacion I2C por que eran muchas lecturas
  // Serial.print("SubTem:");
  // Serial.println(tBME);
  if(tBME >= 38.10 or tDHT >= 38.10){
